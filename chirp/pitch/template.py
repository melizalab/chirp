...
class harmonic(object):
    ...
    def make_template(self, lobes, decay, neg_ampl, neg_width):
        ...
        A,p = normalize_lobes(A, range(lobes), decay)
        ...
        B = normalize_lobes(B, range(1,lobes*2,2), 1.0) * -neg_ampl
...
def normalize_lobes(psd, lobes, decay, thresh=1e-10, between=None):
    ...
    lobes:  a list of the lobes to keep
    ...
    for i,val in enumerate(psd):
        if lobecount > maxlobes: break
        if csum > thresh:
            if val > thresh:
                csum += val
            else:
                R = slice(lobestart,i)
                if lobecount in lobes:
                    out[R] = psd[R] / csum * decay**lobecount
                csum = 0
                lobecount += 1
                lobepos.append((i + lobestart)/2)
        elif val > thresh:
            lobestart = i
            csum = val
    return out,lobepos
